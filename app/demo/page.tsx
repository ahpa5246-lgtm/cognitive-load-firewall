...
export default function DemoPage() {
	...
	function adapt() { setFeedback(null); setResult(runAdaptation({ content, tolerance: defaultTolerance(), mode })); setView("adapted"); }
	return <main className="app-shell">
		...
		<section className="workspace" aria-label="Adapt content workspace">
			...
			<div className="results-panel">{result.blocked ? <div className="safety-notice" role="alert"><span className="label">Safety pause</span><h2>Pause and seek urgent help.</h2><p>{result.safety.message} This tool cannot determine severity and will not provide productivity advice for this message.</p></div> : <>
				...
				<div className="aftercare panel"><div><span className="label">04 / Quick check-in</span><h2>Was this easier to process?</h2></div><div className="feedback-buttons">{["Much easier", "A little easier", "No difference", "Harder"].map((label) => <button key={label} onClick={() => { setFeedback(label); localStorage.setItem("clf-history", JSON.stringify([demoArtifact.title, ...(JSON.parse(localStorage.getItem("clf-history") || "[]"))].slice(0, 5))); }} className={feedback === label ? "selected" : ""}>{label}</button>)}</div>{feedback && <p className="confirmation" role="status">Thanks. Your preference was noted for this demo session.</p>}<ActionLink href="/session" secondary>Start a recovery session</ActionLink></div>
				<details className="receipt panel"><summary>Why was this changed? <span>Decision receipt</span></summary><div className="receipt-grid"><p><strong>Provider</strong> {result.receipt.provider}</p><p><strong>Rules</strong> {result.receipt.rulesTriggered.length || "None"}</p><p><strong>Fidelity</strong> {result.fidelity.safe ? "Critical tokens preserved" : "Review original"}</p><p><strong>Safety</strong> Passed before adaptation</p></div><p>{result.receipt.explanation}</p></details>
			</>}</div>
		</section>
	</main>;
}
