...
export default function DemoPage() {
  ...
  function recordFeedback(label: string) {
    setFeedback(label);
    addHistory({ title: demoArtifact.title, mode, feedback: label });
    const difficulty: FeedbackDifficulty | undefined = ({ "Much easier": undefined, "A little easier": undefined, "No difference": "too_much_at_once", Harder: "too_much_at_once" } as Record<string, FeedbackDifficulty | undefined>)[label];
    if (difficulty) {
      const changed = adjustProfile(profile, difficulty);
      setProfile(changed.profile);
      writeProfile(changed.profile);
      setAdjustment(`${changed.adjustment.reason} Preference changed from ${changed.adjustment.previous} to ${changed.adjustment.next}. Adapt again to see the interface respond.`);
    }
  }
